...
function addTrio(el) {
    ...
    if (trio_container.children.length > 2)
        trio_container.querySelector('span.remove').classList.remove('disabled');
    return;
}
...
function remove(el) {
    ...
    // Case for module
    if (el.parentElement.dataset.module === 'true')
        return removeModule(el);
    // Case for trio
    removeTrio(el);
}
function removeModule(el) {
    // Get last module
    let last_module = document.getElementsByClassName('module');
    last_module = last_module[last_module.length - 1];
    // Remove it
    last_module.remove();
    // Check if there is only 1 question to add disabled to the remove btn
    if (document.getElementsByClassName('modules')[0].children.length <= 1)
        el.classList.add('disabled');
    return;
}
function removeTrio(el) {
    // Get the trio container
    const trio_container = el.parentElement.parentElement.parentElement;
    // Get last trio
